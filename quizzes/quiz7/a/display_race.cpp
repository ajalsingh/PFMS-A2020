...
void DisplayRace::updateDisplay(std::vector<Car> cars){
    ...
    double full_Circ = 2*M_PI;
    
    for (unsigned int i=0;i<cars.size();i++){
        // number of full loops completed
        int loops = cars.at(i).getOdometry()/(circumf);

        // distance of current vehicle position from the full loop
        double arc = cars.at(i).getOdometry()- (circumf*loops);

        // angle of vehicle from full loop
        double angle = full_Circ*(arc/circumf);

        // vehicle x and y coordinates
...
//       you can compute the position x,y using the radius and the angle
        ...
        cv::putText(track_, cars.at(i).getModel(), cv::Point(MAP_CENTRE+x,MAP_CENTRE+y),
                    cv::FONT_HERSHEY_SIMPLEX, 0.5, CLR_TEXT);
