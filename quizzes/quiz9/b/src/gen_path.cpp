...
void CallBackFunc(int event, int x, int y, int flags, void* param)
{
    ...
    PoseArrayBuffer* poseArrayBuffer = ((PoseArrayBuffer*)(param)); // 1st cast it back, then deref
     ...
     if  ( event == cv::EVENT_LBUTTONDOWN )
     {
          ...
          pose.position.x=(double(x)-(centre))*poseArrayBuffer->resolution + 90 + poseArrayBuffer->vehPose.position.x;
          pose.position.y=-(double(y)-(centre))*poseArrayBuffer->resolution - 90 + poseArrayBuffer->vehPose.position.y;
...
public:
    ...
    : nh_(nh), it_(nh)
    {
        ...
        posePublisher_ = nh_.advertise<geometry_msgs::PoseArray>("robot_0/path", 100);
