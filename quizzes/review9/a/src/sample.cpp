             ...
             a4_setup::RequestGoal::Response &res)
{
  ...
  if (poseDataBuffer_.poseDeq.size() > 0) {
      pose = poseDataBuffer_.poseDeq.back();
  }
  ...
  //Find  the max and min value of x and y
  double xmin = (pose.position.x - (mapSize*resolution_/2.0));
  double xmax = (pose.position.x + (mapSize*resolution_/2.0));
  double ymin = (pose.position.y - (mapSize*resolution_/2.0));
  double ymax = (pose.position.y + (mapSize*resolution_/2.0));
