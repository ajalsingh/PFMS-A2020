...
void DisplayRace::updateDisplay(std::vector<Car*> cars){
    ...
    std::vector<double> carAngularPosition;

    for (auto i = cars.begin(); i != cars.end(); i++){
        int loops = ((*i)->getOdometry())/(2*M_PI*RADIUS);
        double carAngle = (((*i)->getOdometry())-loops*(2*M_PI*RADIUS))/RADIUS;
        carAngularPosition.push_back(carAngle);
    }
    ...
    for (unsigned int i=0;i<cars.size();i++){
        double x = RADIUS*sin(carAngularPosition.at(i));
        double y = RADIUS*cos(carAngularPosition.at(i));
...
//       you can compute the position x,y using the radius and the angle
      ...
      cv::putText(track_, cars.at(i)->getMake(), cv::Point(MAP_CENTRE+x,MAP_CENTRE+y),
                  cv::FONT_HERSHEY_SIMPLEX, 0.5, CLR_TEXT);
